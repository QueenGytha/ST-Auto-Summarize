...
export async function sendLLMRequest(profileId, prompt, operationType, options = {}) {
  ...
  try {
    ...
    debug(SUBSYSTEM.CORE, `[LLMClient] Sending ${messagesWithMetadata.length} messages, ${actualPayloadTokens} tokens (actual ST tokenizer count)`);
    debug(SUBSYSTEM.CORE, `[LLMClient] Token breakdown matches actual payload: ${tokenBreakdown.total} tokens`);
