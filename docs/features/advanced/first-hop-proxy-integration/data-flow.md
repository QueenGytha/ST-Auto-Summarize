...
When a user sends a chat message with the feature ENABLED:

1. User types message in ST UI
2. ST fires CHAT_COMPLETION_PROMPT_READY event
3. eventHandlers.js checks isMetadataInjectionEnabled()
4. If enabled, injectMetadataIntoChatArray() prepends metadata to system message
5. ST sends modified chat array to API
6. Proxy receives request with metadata block
7. Proxy extracts and logs metadata (character, operation)
8. Proxy strips metadata from messages
9. Proxy forwards clean messages to LLM
10. LLM generates response (never sees metadata)

## Recap Generation Flow

When user generates a scene recap with the feature ENABLED:

1. User clicks Generate Scene Recap button
2. Extension queues SCENE_RECAP operation
3. Handler calls setOperationSuffix('-START-END')
4. Handler calls generateSceneRecap()
5. Recap calls wrappedGenerateRaw() with prompt string
6. Interceptor calls determineOperationType() from stack
7. Interceptor retrieves getOperationSuffix()
8. Interceptor combines: 'generate_scene_recap' + '-42-67'
9. Interceptor calls injectMetadata() with full operation
10. Metadata injector checks isMetadataInjectionEnabled()
11. Creates metadata block with operation: 'generate_scene_recap-42-67'
12. Formats as XML and prepends to prompt
13. ST sends request to API with metadata
14. Proxy extracts metadata with operation suffix
15. Proxy logs request with character name and message range
16. Proxy strips metadata and sends to LLM

## Operation Type Detection

The interceptor determines operation type by analyzing the call stack:

Priority (checked in order):
1. detectSceneBreak -> detect_scene_break
2. generateSceneRecap (not runningScene) -> generate_scene_recap
3. runningSceneRecap.js with combine -> combine_scene_with_running
4. runningSceneRecap.js alone -> generate_running_recap
5. validateRecap -> validate_recap
6. lorebookEntryLookup -> lorebook_entry_lookup
7. lorebookEntryDeduplication -> resolve_lorebook_entry
8. createLorebookEntry -> create_lorebook_entry
9. mergeLorebookEntry -> merge_lorebook_entry
10. updateRegistry -> update_lorebook_registry
11. bulk_registry_populate -> populate_registries
12. No match -> chat (default)
13. Error -> unknown (fallback)

## Metadata Injection for Strings

1. Check isMetadataInjectionEnabled()
2. If false, return original prompt
3. If true, create metadata block
4. Format as XML with JSON content
5. Prepend to prompt
6. Return modified prompt

## Metadata Injection for Arrays

1. Check isMetadataInjectionEnabled()
2. Get existing operation (if any)
3. Handle existing metadata:
   - If replaceIfChat=true and existing is 'chat' type, replace it
   - If existing is specific type, keep it (don't override)
   - If no existing, proceed to create
4. Find first system message in array (or create one)
5. Strip old metadata if replacing
6. Prepend new metadata to system message content
7. Modify array in place
8. Return

## Proxy Parsing

When proxy receives request with metadata:

1. Extract messages array from request
2. For each message, check content for ST_METADATA
3. If found, parse JSON and extract metadata object
4. Strip metadata from message content
5. Collect all metadata and cleaned messages
6. Call extract_character_chat_info()
7. Parse chat field to get character name and timestamp
8. Get operation from metadata
9. Log request with structured data
10. Forward cleaned messages to LLM

## Example: Chat Message Request

User sends chat with metadata enabled:
...
PROXY RECEIVES: Extracts metadata, logs, removes it
...
## Example: Recap with Range

Scene recap for messages 42-67:
...
PROXY EXTRACTS:
character: "Senta"
timestamp: "2025-11-01@20h29m24s"
operation: "generate_scene_recap-42-67"

CAN NOW:
- Log as recap operation
- Know message range from suffix
- Track by character
- Categorize in logs
