...
Forked from [SillyTavern-MessageSummarize](https://github.com/qvink/SillyTavern-MessageSummarize/).
...
- **Combined summary**: Optionally merges all message summaries into a single, coherent narrative summary, removing repetition and highlighting key events.
...
- Summarizing messages individually gets more accurate summaries and is less likely to miss details.
...
- Install the extension in SillyTavern using the GitHub link: https://github.com/QueenGytha/ST-Auto-Summarize
...
- `/toggle_memory`: Toggles the extension on and off for the current chat. Can also provide a boolean argument to toggle the extension directly.
- `/toggle_memory_display`: Toggles the display of summaries below each message.
...
- `/summarize`: Summarizes the nth message in the chat (default to most recent message).
- `/summarize_chat`: Performs a single auto-summarization on the chat, even if auto-summarization is disabled.
- `/stop_summarization`: Stops any summarization currently running.
- `/remember <n>`: Mark the nth message for long-term memory, summarizing it if not already.
- `/force_exclude_memory <n>`: Toggles the inclusion of the summary for the nth message.
...
- **No need to pause roleplay**: The summary prompt is completely independent and will only send what you see in the edit window.
- **I don't recommend reasoning**: Reasoning models can summarize fine, but they tend to be verbose and slow.
...
- **Summaries seem to be continuing the conversation rather than summarizing:** Make sure you are using the correct template for your model, and that system messages are properly distinct from user messages (the summaries use a system prompt). Some default instruct templates may not have anything defined for the "System message sequences" field - that should be filled out. You can also try toggling "Nest Message in Summary Prompt" in the settings.
...
- **When I use a different completion preset for summaries, my regular completion preset gets changed after summarizing:** When a summary is generated, the extension temporarily switches completion presets, which discards any unsaved changes you might have made to your current completion preset. The same applies to connection profiles.
...
- Validation prompts may have a high false positive rate due to meta-commentary in LLM outputs.
...
- 'Auto' option for injection placement, adjusting based on summary size
- Version the individual/combined summaries, with the option to choose between them
- The validation prompts have a very large false positive rate, due to meta-commentary. Possible solution: convert summaries to JSON objects for ease of validation.
